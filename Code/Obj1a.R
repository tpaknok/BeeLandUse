...
m_SAC <- lm(SR~log10(Dist+1),data=SAC_data)
summary(m_SAC)
...
tableS1_1 <- as.data.frame(summary(mTD2,details=T)$beta_table)

AIC(mTD2)

mPD.S2 <- fitme(log10(qPD.S)~Agr1+Urb1+abundanceMethod+PC1+PC2+(Agr1+Urb1||studyID)+Matern(1|Long + Lat %in% studyID),data=analysis.df.PD,
                family=gaussian,weights.form=~SC,method="ML",verbose=c(TRACE=TRUE),control.dist=list(dist.method="Earth"))
summary(mPD.S2,details=T)
plot(simulateResiduals(mPD.S2))
tableS1_2 <- as.data.frame(summary(mPD.S2,details=T)$beta_table)

AIC(mPD.S2)

analysis.df.PD$Covariate.Est.Shannon <- log10(analysis.df.PD$Exclude.AM.Est.Shannon)
...
tableS1_3 <- as.data.frame(summary(mPD.S.controlled2,details=T)$beta_table)
...
newdataTD[,c("predicted_y","predVar_0.025","predVar_0.975")] <- 10^newdataTD[,c("predicted_y","predVar_0.025","predVar_0.975")]
...
newdataPD = expand.grid(Agr1=c(0,1),Urb1=c(0,1),abundanceMethod="Abundance",PC1=0,PC2=0)
...
newdataPD[,c("predicted_y","predVar_0.025","predVar_0.975")] <- 10^newdataPD[,c("predicted_y","predVar_0.025","predVar_0.975")]
...
newdataPD_C = expand.grid(Agr1=c(0,1),Urb1=c(0,1),abundanceMethod="Abundance",PC1=0,PC2=0,Covariate.Est.Shannon=mean(analysis.df.PD$Covariate.Est.Shannon))
...
pPD_C <- ggplot()+
  ...
  xlab("")+
  ylim(-40,20)+
  annotate('text', label=' (d) Phylogenetic', x=-Inf, y=Inf, hjust=0, vjust=1,size=4)+
  scale_colour_manual(values=c("green4","darkgoldenrod2","grey25"))+
